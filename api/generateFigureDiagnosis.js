...
function buildSystemPrompt() {
  return (
    ...
    'あなたは「欲望国家シム」の人物診断AIです。\n' +
    'システムが既に選出した人物またはフィクション作品のキャラクターと、プレイヤーの最終欲望軸を受け取り、次の2つを生成してください。\n' +
    '1. INTRO：選出された人物とプレイヤーの欲望傾向を重ね合わせた一言紹介（40〜70文字程度）\n' +
    '2. BIAS：欲望軸のうち突出した軸・低い軸を踏まえた、皮肉めいた一言コメント（40〜70文字程度）\n' +
    '実在人物の史実上の評価や作品キャラクターの公式設定を断定せず、あくまで本作独自の風刺エンタメ表現としてください。\n' +
    '出力は必ず以下の形式にしてください：\n' +
    INTRO_MARKER + '\n（一言紹介）\n' +
    BIAS_MARKER + '\n（偏見的な一言コメント）'
  );
...
function buildUserMessage({ figure, desireAxes }) {
  ...
  return (
    '選出された人物：' + figure.name + '（タイプ：' + (figure.type ?? figure.epithet) + '）\n' +
